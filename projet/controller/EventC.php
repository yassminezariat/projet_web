...
class EventC {
    function ajouterEvent($event){
        $sql="INSERT INTO event (date,type,description,status,user_id) VALUES (:date,:type,:description,:status,:user_id)";
        ...
        try{
            ...
            $req->bindValue(':status',$event->getStatus());
            $req->bindValue(':user_id',$event->getUserId());
            $req->execute();
    ...
    function afficherEvent(){
        $sql="SELECT Event.*,nom,prenom FROM Event inner join user on Event.user_id=user.id";
        $db = config::getConnexion();
        try{
            $liste=$db->query($sql);
            return $liste;
    ...
    function modifierEvent($event){
        $sql="Update event set date=:date,description=:description,type=:type where id=:id";
        $db = config::getConnexion();
        try{
            $req=$db->prepare($sql);
            $req->bindValue(':date',$event->getDate());
